...
- **Adjacency List**: Instead of a visual representation, an adjacency list can be used. Each connection is to be recorded as: `[Sector A] connects to [Sector B]`. Connections are bi-directional, and every sector in the mech must be reachable from any other sector.
...
- **Allocation Guidelines**: A typical combat mecha consists of roughly 50% defensive systems ([Armor](endworld/mecha/systems/armor) and [Shields](endworld/mecha/systems/defensive)), 10-15% each for [Heat](endworld/mecha/systems/heat), [Energy](endworld/mecha/systems/energy), [Offensive](endworld/mecha/systems/offensive), and [Movement](endworld/mecha/systems/movement) systems, with remaining weight allocated to [Support](endworld/mecha/systems/support) and cargo.
